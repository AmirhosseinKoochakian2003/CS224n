...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        t_len = random.randint(4, int(self.block_size*(7/8)))
        truncated_document = document[:t_len]

        mask_mean_len = int(t_len / 4)
        mask_len = mask_mean_len + random.randint(-mask_mean_len, mask_mean_len)
        mask_idx = random.randint(0, t_len - mask_len)

        prefix = truncated_document[:mask_idx]
        masked_content = truncated_document[mask_idx:mask_idx+mask_len]
        suffix = truncated_document[mask_idx+mask_len:]

        masked_string_before_padding = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        masked_string =  masked_string_before_padding + self.PAD_CHAR * max(0, self.block_size - len(masked_string_before_padding))
