...
def naiveSoftmaxLossAndGradient(
    ...
    yhat = softmax(centerWordVec @ outsideVectors.T)
    y = np.zeros(yhat.shape)
    y[outsideWordIdx] = 1

    loss = -np.log(yhat[outsideWordIdx]) 

    gradCenterVec = (yhat - y) @ outsideVectors
...
def negSamplingLossAndGradient(
    ...
    # counts array is needed because samples can be replicated
    unique_words_indices, indices, counts = np.unique(indices, return_index=True, return_counts=True) 
    U = outsideVectors[unique_words_indices]

    signed_counts = counts.copy()
    signed_counts[indices == 0] = -1 * counts[indices == 0] # terms related to u_o are multiplied to -1
    U[indices != 0] = -1 * U[indices != 0] # for noise words, we use sigmoid(-u_k . v_c)

    S = sigmoid(centerWordVec @ U.T)

    loss = np.sum(-counts * np.log(S))

    gradCenterVec =  counts * (1 - S) @ (-U)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[unique_words_indices] = signed_counts[:, np.newaxis] * np.outer(1 - S, centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        loss_i, gradCenterVec_i, gradOutsideVector_i = word2vecLossAndGradient(centerWordVector, word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_i
        gradCenterVecs[centerWordIndex] += gradCenterVec_i
        gradOutsideVectors += gradOutsideVector_i
